...
## ðŸ”¶ BUILT, NOT DEPLOYED â€” estate themes adopted + index backstop off the cron, 2026-08-13
...
| | |
|---|---|
| Themes | The app styles against the estate `--et-*` contract (guide: `catalog-platform/docs/info/estate-themes.md`). **Retro â€” this app's own look, extracted verbatim â€” stays the default** (`data-default-theme="retro"`); apple/cyberpunk selectable in the cog's new Theme group. Storage moved to `hg_theme`/`hg_mode` with a migrate-once from `bgc-theme` in index.html; `bgc-theme` is never written again |
| Vendored assets | `apps/web/public/assets/` â€” estate-theme.css + theme.js from catalog-platform `cba0397` **plus the games-adoption tokens** (same patch as canonical `e95a32d`); Rajdhani Ã—3 + Share Tech Mono woff2 + OFL join the committed fonts. âš ï¸ Re-vendoring from canonical â‰¥`e95a32d` is safe and also brings the `classic` theme; the games cog offers three themes either way until someone adds it |
| âš ï¸ Cache | `_headers` gives estate-theme.css/theme.js `no-cache` â€” they are NOT content-hashed; without that rule the `/assets/*` immutable line would pin the first theme css a phone ever saw for a year |
| Index backstop | No longer rides the half-hourly cron (it silently failed 3 consecutive ticks 2026-08-13 while a manual push with the same token succeeded; tails died before catching it). Now rides request traffic: at most one health GET per isolate-hour, **every /api/\* request logs its backstop decision** â€” proof is `wrangler tail` + one unauthenticated `curl /api/health`. After-mutation pushes and the cron's other duties untouched |
| Verified | typecheck all workspaces + vite build (âš ï¸ no test script in this repo); local `wrangler dev` showed `due â†’ skipped (fresh, 836 rows)` then `throttled (next in 60m)`. **Not verified: an attended look at the three themes** â€” token plumbing is checked (no undefined `var()` anywhere), pixels are not |

---

## ðŸ”¶ BUILT, NOT DEPLOYED â€” estate auth adopted in shadow mode, 2026-08-13

Commit `0077a7a`, pushed. Design:
`catalog-platform/docs/info/estate-auth-design.md` Â§3.1/Â§5/Â§14.5.

| | |
|---|---|
| What | Estate membership check wired into `requireAuth`, gated by `ESTATE_CHECK` (`off` \| `shadow` \| `enforce`) â€” **committed as `off`, so deploying this is inert** |
| New build dependency | âš ï¸ This repo now materialises the canonical `estate-auth` module from the sibling `catalog-platform` checkout â€” `scripts/sync-estate-auth.mjs` runs as `predev`/`pretypecheck`/`predeploy` and **fails loudly** if the checkout is missing. The old local verifier in `middleware/auth.ts` was replaced by it (behaviour-identical: the hardened bypass came FROM here) |
| Migration | `0026_estate_cache.sql` â€” two nullable `app_user` columns, plain ADD COLUMN. **Applied LOCAL only; remote apply is a pending owner/dispatcher step**, before the deploy that carries this code |
| Secret | `npm run secret ESTATE_APP_TOKEN_GAMES` (same value the auth Worker holds under that name) â€” without it, shadow logs `config unset` per request and skips |
| Reading shadow | `npm run tail --workspace @bgc/worker`, grep `estate shadow:`; the lines that matter carry **`WOULD-DENY`** â€” expect zero for household members before anyone flips `enforce` |
| Default-grant | `viewer` (the smaller guest role, on purpose â€” rating stays a local upgrade, preserving 0023/0024). Written only in `enforce`; shadow logs the would-grant |
| Untouched | `OWNER_EMAILS` recovery hatch (runs before the estate check), the rate limiter, every route and capability gate |
| Verified | typecheck (âš ï¸ no test script in this repo) + 11 `wrangler dev` probes against local D1 with a mock `/seen`: off inert; shadow logs revoked as WOULD-DENY while answering 200, rides a stale cache through an outage, would-grants without writing; enforce grants/403s/503s correctly and serves the standing owner through an outage |

---

## â­ï¸ ON HOLD â€” disposal & copy history
...
ðŸ“„ **The plan is written: [`info/copy-status-history.md`](info/copy-status-history.md).**
...
history of them items. Map this feature for tomorrow's reset."* â€” the owner.

The four things that decide the shape, all in that doc:

1. **`lent` and `sold` have existed since 0001 and have never been used** â€” 0
...
2. **One question has to go to the owner first.** They said `lent` should stop
   ...
   shopping list â€” the exact "bought twice" failure `preordered` counts as held
   to avoid. Recommendation and both readings are in Â§2.
...
4. **History must not cascade.** `copy` cascades from `item`, so the obvious FK
   erases the record that you ever owned the thing â€” the one fact being kept.

---

---

## â­ï¸ Superseded note â€” marking things sold or given away
...
different thread."* â€” the owner, 2026-08-09. **Not built. Do not start it
without reading this first.**

âš ï¸ **Half of it already exists, so this is probably not the feature it sounds
...
reach the owner before writing any code â€” the likely answers are
discoverability (where `CopyEditor` is reachable from) or that neither `sold`
nor `lent` means *given away*.

| Known | |
|---|---|
| Item 303 | `The Binding of Isaac: Four Souls - Gold Box Expansion`, copy 298, still `owned` â€” the owner says it is gone |
...
âš ï¸ The completeness feature reads `owned/lent/preordered` as **held**. Any new
...
The Kickstarter **Gold Box** contains the base game plus **five** expansions â€”
Dinolings (361645), Mythlings (361646), Techlings (376383), Multicolor (376384,
sold as *"Dual Color"*) and The Meaning of Life (376382) â€” plus the exclusive
...
direct evidence for the claim made earlier in this file â€” the card's "I have it"
...
after the campaign closed, so no stretch-goal content ever shipped to them â€” the
general rule for this catalog, since stretch goals reach backers only.

### The non-English edition filter, shipped â€” 2026-08-09

| | |
|---|---|
| Live worker version | **`2f1a26a3-c60c-4292-850e-167d58a3935a`** |
| Roll back to | `5e4538ea-2598-4cdc-b4b3-0c04da1f6f93` |
| Commit | `72b0c63` â€” **no migration**, computed on read |
...
components move; Catan's expansions fall **84 â†’ 57**, Codenames 29 â†’ 19.

âš ï¸ **The false positive the owner predicted was real, and it is `die`** â€” the
...
components are English-named regional one-offs â€” `Catan Geographies: Mallorca`,
...
### The bundled-components bypass, shipped â€” 2026-08-09
...
âš ï¸ **`wrangler d1 migrations apply --remote` does not work on this account.** It
answers `7403 â€” the given account is not valid or is not authorized to access
...
âœ… **Confirmed live by the owner: "What else exists" loads.** That check had to
come from a signed-in browser â€” Access intercepts every route at the edge, so a
...
### All 17 Dice Throne sleeve components marked bundled â€” 2026-08-09

*"Run came bundled on all sleeve stuff for dice throne."* â€” the owner.

**Uncertain rows 13 â†’ 8, and the 8 left are all correct wishlist entries.** The
name-only group is now **empty catalog-wide** â€” every remaining `uncertain` is
...
`missing`, and no amount of matcher tuning would have surfaced them â€” which is
the argument for the bypass existing rather than the floor being lowered.

### Ten more `bgg_id`s, and the owner correcting two of my matches â€” 2026-08-09

Uncertain rows **20 â†’ 13**. The remaining 13 are 8 correct wishlist entries and
the 5 Dice Throne sleeves, which no id can fix (see the section below).

| Item | â†’ | Component | Note |
|---|---|---|---|
...
| 425 | 429978 | Cyberpunk: Johnny Silverhand **& NCPD** | âš ï¸ **not** 430049 |
...
âš ï¸ **Two traps, both caught by the owner rather than by the matcher.**
...
**127**, `Downfall - Character Playmats` â€” a different, unreleased product for a
...
**Cyberpunk 2077.** BGG lists **two** Johnny Silverhand entries â€” 430049
...
**On "it's a preorder, should we wait?" â€” no, and the schema already says so.**
...
Ran the real `buildCompleteness` over all **139 checked games** at once â€” same
...
|---|---|---|
| **A** Already on your wishlist | **8** | Correct. Ark Nova Ã—3, Fractured Sky Ã—4, Here to Slay Ã—1. Leave alone |
| **B** In the catalog, no copy recorded | **0** | Nothing in this state |
| **C** Name matched, no `bgg_id` | **12** | The only fixable group â€” and only *some* of it |

Group C splits again, which is the finding worth keeping:

**Six are genuine** â€” set the id and they become `held`: Twisted Cryptids
...
**Five are the matcher being loose, and an id would be wrong.** One row â€”
*Dice Throne: Minimalist Card Sleeves* â€” is hinted against **three different
...
bag* is hinted against *Expansions Dice Tray* â€” a bag is not a tray.

âš ï¸ **The Dice Throne sleeves are the interesting failure.** `withoutGamePrefix`
...
"Card Sleeves - Raveness" â€” mostly shared words describing a product category
...
(`E28093`, a real en dash â€” verified with `hex()` because the console lied about
...
Promos and collectibles are now split out of the completeness figures â€” the
...
76 â†’ 19, Mysterium's 8 â†’ 2. Catan barely moves (82 â†’ 80) and that is known â€”
...
inside the disclosure with both buttons gone, and puts *"Â· you have N"* on the
header. It does **not** move the "X of Y" figure â€” out of the numerator and the
denominator alike, which is the same bargain third-party has always had.

### âš ï¸ Work in flight at the moment this was written
...
| `scratchpad/bgg-audit-2026-08-08.{md,tsv}`, `scratchpad/bgg-audit/` | A **read-only** audit of every `bgg_id` â€” a map to verify before anything is applied. Nothing written to production |

Neither is deployed and neither needs to be. **If any of it looks
half-written, `git checkout --` the file rather than trying to finish it** â€”
...
â€¦except Access intercepts that at the edge, so it has to come from a
signed-in browser console instead â€” see the "Verifying anything" section of
...
| Item 826 "The Settlers of Catan" | **deleted** â€” a real duplicate of item 54 |
...
| **15 `series` labels** | across ~470 rows â€” see the family sweep section |
| **`same_family` 65 â†’ 90** | 25 edges from the sweep, plus Slam Throne |
| Items **858, 859** | Here to Slay: Warriors & Druids / Berserkers & Necromancers expansions, added `owned` â€” implied by accessories already held |
| Item **277 deleted, merged into 833** | *Casting Shadows: Expansion Pack* was the campaign name for what retailed as **The Ice Storm Expansion**. Confirmed from the Kickstarter reward tier ("Kickstarter Exclusive Edition + Expansion" â†’ "Casting Shadows Expansion Pack"), and a copy note from 08-06 had already recorded the same conclusion. The pledge provenance moved onto copy 808; Casting Shadows now has exactly two expansions, both linked |
| **`bgg_id` 197 â†’ 232** | 35 audited matches applied in two batches â€” 23 exact, then 12 near-matches after the owner checked each against its BoardGameGeek page |

### What still wants a person

| | |
|---|---|
| â³ **`game_component` is filling â€” finish it** | Was 0 all day; the owner started the backfill 2026-08-08 and it is at **1,217 components / 100 games checked**. Roughly 7 more runs of `await (await fetch('/api/components/backfill',{method:'POST'})).json()` from a signed-in console, or the Sunday 05:41 UTC cron does the rest unattended. **Until it finishes, the completeness page reads "Not checked yet" and the collapsible sections built today are invisible** â€” they are not broken, they have nothing to show |
| **Accessory implies the game â€” a sweep worth doing** | Proved on Here to Slay: six accessories (Warriors & Druids Ã—3, Berserkers & Necromancers Ã—3) existed with no expansion row behind them, and both expansions were real. Now added as items **858** and **859**. You own **221 accessories against 186 expansions**, so this almost certainly holds on other lines. Banner Quest is the control case â€” accessory *and* expansion both present |
| âœ… **The BGG audit is CLOSED** | All 806 rows audited and resolved. **`bgg_id` coverage went 197 â†’ 232.** Every remaining row without one has a *recorded reason*, not a gap. The map is `scratchpad/bgg-audit-2026-08-08.tsv`; the decision sheet `scratchpad/bgg-audit-review.md` is **spent** â€” its "nothing has been applied" line is historical. Both are safe to delete once you trust the result |
| âœ… **All five SUSPECT rows resolved** | 114 Deadpool (id is right; see [`dice-throne-shape.md`](dice-throne-shape.md) â€” the box owns the id) Â· 496 Yeti or Not (id is the game, our name says which version) Â· 801 Go Fish (the `Traditional` marker) Â· 56 and 68 (publisher-spelling noise). **Do not re-open these** |
| âœ… **`copy.edition_id` stays null â€” settled, not a gap** | The owner, 2026-08-08: *"that'll probably be null forever, it's a hard thing to find and I don't super care to track it down."* 1,063 edition rows exist with 768 BGG version ids, so the catalog knows which printings **exist** and deliberately does not record which one is on the shelf. **Do not re-flag this as missing data.** Consequences, all fine: the cover picker is unaffected (it sets `thumbnail_url`, not `edition_id`), and it independently kills the "grab the closest edition" half of any BoardGameGeek sync â€” you cannot match a printing you never recorded. Under 1% of BGG users populate that field either |
...
| âš ï¸ Three orphaned non-base rows sitting as their own roots | 823 Dark Moon: Shadow Corporation (`expansion`, wants nesting under 790), 842 Tiny Epic Dungeons Adventures: The Phantom Voyage (`expansion`, under 840), 830 Scales of Fate Metal Upgrade Kit (`accessory`, under 91) |
| HELLDIVERS 2: Mystery Expansions (item 414) | rename from the box when the pledge ships; deliberately a placeholder |
| Dice Throne playmats | count them on the shelf â€” see `scratchpad/dice-throne-playmats.md` |
| âš ï¸ Excursion Tiles 1 (117) says **2024** | its campaign actually ran **2025-08-06 to 2025-08-27** (543 backers, $24,488), delivering Oct 2025. 2024 has no evidence behind it. **Left alone deliberately** â€” the owner has settled both these years by hand; it is a one-line `UPDATE item SET year_published = 2025 WHERE id = 117` if they agree |

### Login is now Google SSO â€” and the email PIN was deliberately kept
...
switch untouched â€” `upsertUserOnLogin` matches on the lowercased email, and
Google returns the same string the PIN flow did.

### BoardGameGeek has no write API â€” settled 2026-08-08
...
It would also sync **142 of 573 owned items (25%)** â€” base games are 114/131,
