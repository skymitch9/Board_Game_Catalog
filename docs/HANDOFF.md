...
| ⚠️ **Decisions waiting on the owner** | **[`scratchpad/bgg-audit-review.md`](../scratchpad/bgg-audit-review.md)** — 35 `bgg_id` matches to approve, and two rows (114 Deadpool, 496 Yeti or Not) only the owner can settle. The SQL is written out and **nothing has been applied** |
| ⚠️ **The `bgg_id` gap, now measured** | All 806 items audited: **192 of 197** existing ids CONFIRMED, 5 SUSPECT, 143 PROPOSED, 466 UNMATCHED (323 of those with a *named* rejected near-miss). The full map is `scratchpad/bgg-audit-2026-08-08.tsv`. This gap is what keeps `game_component` empty and the completeness feature answering "No data" |
